class AlbumPage {
  ...
  loadAlbumData() {
    this.currentAlbum = DataService.getAlbumByName(
      this.tracks,
  ...
  populateTracklist() {
    ...
    trackRows.forEach((row, index) => {
      if (this.currentAlbum.tracks[index]) {
        ...
        if (trackDate)
          trackDate.textContent = new Date(
            track.uploadDate,
          ).toLocaleDateString();
  ...
  setupEventListeners() {
    ...
    const forwardBtn = document
      .querySelector('button[onclick*="Forward"]')
      ?.closest(".buttonsb");
    const backBtn = document
      .querySelector('button[onclick*="Back"]')
      ?.closest(".buttonsb");
    ...
    if (shuffleBtn) {
      shuffleBtn.onclick = () => {
        ...
        showNotification(
          isShuffled ? "Aleatório ativado" : "Aleatório desativado",
          "info",
        );
      };
    }
    ...
    // Botão Loop
    const loopBtn = document
      .querySelector('img[src*="Loop"]')
      ?.closest(".buttonsb");
    if (loopBtn) {
      loopBtn.onclick = () => {
